...
const submitCreateDb = () => {
    createDbForm.post(route('databases.store', {
        connection: props.selectedConnectionId
    }), {
        preserveScroll: true,
        onSuccess: () => closeModal(),
    ...
    <AuthenticatedLayout>
        ...
        <div class="flex flex-col md:flex-row h-[calc(100vh-65px)] overflow-hidden">
            ...
            <nav
                ...
                :class="{
                    'w-full md:w-80': true,
                    'hidden md:block': mobileView !== 'nav'
                }"
            >
                            ...
                            <li v-for="conn in userConnections" :key="conn.id">
                                ...
                                <div v-if="conn.id === selectedConnectionId" class="ml-4 mt-1 pl-2 border-l-2 border-gray-200 dark:border-gray-700 space-y-1">
                                    ...
                                    <div class="flex justify-between items-center px-2 py-1">
                                        <span class="text-xs font-bold text-gray-400 uppercase">Bancos</span>
                                        <button @click.stop="openCreateDbModal" class="text-blue-500 hover:text-blue-700" title="Novo Banco">
            ...
            <main
                class="flex-1 p-6 bg-gray-100 dark:bg-gray-900 overflow-y-auto min-w-0"
                :class="{
                    'hidden md:block': mobileView !== 'content'
                }"
            >
                ...
                <div v-else-if="!selectedConnectionId && !connectionError" class="flex flex-col items-center justify-center h-full text-center p-10">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 max-w-md">
                        Selecione uma conexão no menu à esquerda para começar.
                    </p>
                </div>
            </main>
            ...
            <Modal :show="showCreateDbModal" @close="closeModal">
                <form @submit.prevent="submitCreateDb" class="p-6 dark:bg-gray-800">
                    <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Criar Novo Banco de Dados</h2>
                    <div class="mb-4">
                        <InputLabel for="new_db_name" value="Nome do Banco" />
                        <TextInput id="new_db_name" type="text" v-model="createDbForm.name" class="mt-1 block w-full" required placeholder="ex: meu_projeto" />
                        <InputError :message="createDbForm.errors.name" class="mt-2" />
                    </div>
